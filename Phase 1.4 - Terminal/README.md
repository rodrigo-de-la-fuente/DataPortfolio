...
## Atajos
- Autocompletado con `TAB`: El autocompletado es una de las funciones más útiles de la terminal. Supongamos que existe una carpeta llamada `DataPortfolio`. Puedes escribir `cd Dat` y pulsar `TAB`. La terminal completará automáticamente `cd DataPortfolio`. Esto evita tener que escribir nombres largos y reduce los errores. Pero supongamos que tienes muchas carpetas que empiezan por D y escribes `cd D`, y pulsas `TAB`. Como existen varias opciones, la terminal puede no saber cuál quieres. Si vuelves a pulsar `TAB`, mostrará las coincidencias.

- Flechas `↑` y `↓`: La terminal guarda los comandos que has ejecutado. Con `↑` recuperas el comando anterior y con `↓` avanzas de nuevo por el historial.
